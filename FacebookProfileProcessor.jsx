 ...
 * Transforms Marketplace URLs, extracts profile data, and exports results
 */
export default function FacebookProfileProcessor() {
  ...
  // Extract profile data using Claude API
  const extractProfileData = async (cleanUrl, profileId) => {
    try {
      const response = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: "user",
            content: `Analyze this Facebook profile URL: ${cleanUrl}. 
            
Based on the profile ID ${profileId}, provide any publicly available information you can determine.
Return ONLY valid JSON in this exact format with no additional text:
{"name":"Unknown","bio":"Not available","followers":"Unknown","notes":"Profile ID: ${profileId}"}`
          }]
        })
      });
  ...
  const startProcessing = async () => {
    ...
    for (let i = 0; i < pendingProfiles.length; i++) {
      ...
      while (attempts < 3 && !result) {
        try {
          result = await extractProfileData(profile.clean_url, profile.profile_id);
          break;
        } catch (error) {
          attempts++;
          if (attempts === 3) {
      ...
      const updatedProfiles = profiles.map(p => {
        if (p.id === profile.id) {
          return {
            ...
            error: result.status === 'error' ? result.notes : null
          };
        }
        return p;
      });
  ...
  const exportCSV = () => {
    const headers = ['Profile ID', 'Name', 'Bio', 'Followers', 'Clean URL', 'Status', 'Fetched At', 'Error'];
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900 p-4 md:p-8">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-400 text-lg">Transform and extract data from Facebook Marketplace URLs</p>
        </div>
                ...
                <tbody>
                  {filteredProfiles.map((profile, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-slate-200 font-medium">{profile.name || '-'}</td>
                      <td className="px-6 py-4 text-slate-400 text-sm max-w-xs truncate">
                        {profile.bio || '-'}
                      </td>
                      <td className="px-6 py-4 text-slate-300 text-sm">{profile.followers || '-'}</td>
