...
class ProviderManager:
    ...
    def __init__(self, config: Optional[FacebookConfig] = None, db_path: str = "facebook_profiles.db"):
        ...
        self._schema_version = self._check_schema_version()

        # Only initialize provider if schema supports it
        if self._schema_version >= 5:
            ...
            logger.warning(f"Schema v{self._schema_version} - limited provider mode (v5+ required for full features)")
            self._initialized = False

    def _check_schema_version(self) -> int:
        """Check database schema version for API support"""
        try:
    ...
    def is_legacy_schema(self) -> bool:
        """Check if using legacy schema (no full API support)"""
    ...
    def get_status(self) -> Dict:
        ...
        if self.is_legacy_schema():
            return {
                ...
                'error': f'Schema v{self._schema_version} - requires v5+ for full provider features'
            }
